...
mod bug {
    ...
    impl Bug {
        ...
        pub fn update_timestamp(&mut self) {
            ...
            ink::env::debug_println!("Last timestamp = {:?}", self.last_timestamp);
            ink::env::debug_println!("Current timestamp = {:?}", current_timestamp);
            ink::env::debug_println!("Delta timestamp = {:?}", timestamp_delta);

            let multiplication =
                match 1000000000000000000000000000000u128.checked_mul(timestamp_delta) {
                    Some(x) => x,
                    None => {
                        ink::env::debug_println!("Multiplication returned none");
                        0
                    }
                };

            ink::env::debug_println!("Multiplication = {:?}", multiplication);

            let division = match multiplication.checked_div(100) {
                Some(x) => x,
                None => {
                    ink::env::debug_println!("Division returned none");
                    0
                }
            };

            ink::env::debug_println!("Division = {:?}", division);
